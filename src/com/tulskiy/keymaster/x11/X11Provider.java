...
public class X11Provider extends Provider {
	...
	public void init() {
		Runnable runnable = new Runnable() {
			public void run() {
				Logger.get().log(Level.INFO, "Starting X11 global hotkey provider");
				...
				while (listening) {
						...
						if (event.type == KeyPress) {
							...
							for (X11HotKey hotKey : hotKeys) {
								...
								if (hotKey.code == (byte) xkey.keycode && hotKey.modifiers == state) {
									Logger.get().log(Level.INFO, "Received event for hotkey: " + hotKey);
									fireEvent(hotKey);
									break;
								}
							}
					...
					synchronized (lock) {
						if (reset) {
							Logger.get().log(Level.INFO, "Reset all hotkeys.");
							resetAll();
							reset = false;
							lock.notify();
						}
						...
						while (!registerQueue.isEmpty()) {
							...
							Logger.get().log(Level.INFO, "Registering hotkey: " + hotKey);
							if (hotKey.isMedia()) {
				...
				Logger.get().log(Level.INFO, "Thread - stop listening");
			}
		};
	...
	private void register(X11HotKey hotKey) {
		...
		if (code == 0) {
			Logger.get().log(Level.WARNING, "Could not find mapping for " + hotKey.keyStroke);
			return;
		}
	...
	class ErrorHandler implements XErrorHandler {
		public int apply(Pointer display, XErrorEvent errorEvent) {
			...
			while (buf[len] != 0)
				len++;
			Logger.get().log(Level.WARNING, "Error: " + new String(buf, 0, len));
			return 0;
		}
	}
