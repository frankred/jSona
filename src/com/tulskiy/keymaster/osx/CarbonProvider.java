...
public class CarbonProvider extends Provider {
	...
	public void init() {
		thread = new Thread(new Runnable() {
			public void run() {
				synchronized (lock) {
					Logger.get().log(Level.INFO, "Installing Event Handler");
					...
					if (status != 0) {
						Logger.get().log(Level.WARNING, "Could not register Event Handler, error code: " + status);
					}

					if (eventHandlerReference.getValue() == null) {
						Logger.get().log(Level.WARNING, "Event Handler reference is null");
					}
					listen = true;
					while (listen) {
						if (reset) {
	...
	private void resetAll() {
		Logger.get().log(Level.INFO, "Resetting hotkeys");
		for (OSXHotKey hotKey : hotKeys.values()) {
			int ret = Lib.UnregisterEventHotKey(hotKey.handler.getValue());
			if (ret != 0) {
				Logger.get().log(Level.WARNING, "Could not unregister hotkey. Error code: " + ret);
			}
		}
		hotKeys.clear();
	}
	...
	private void register(OSXHotKey hotKey) {
		...
		if (status != 0) {
			Logger.get().log(Level.WARNING, "Could not register HotKey: " + keyCode + ". Error code: " + status);
			return;
		}

		if (gMyHotKeyRef.getValue() == null) {
			Logger.get().log(Level.WARNING, "HotKey returned null handler reference");
			return;
		}
		hotKey.handler = gMyHotKeyRef;
		Logger.get().log(Level.INFO, "Registered hotkey: " + keyCode);
		hotKeys.put(id, hotKey);
	}

	@Override
	public void stop() {
		Logger.get().log(Level.INFO, "Stopping now");
		try {
			synchronized (lock) {
				listen = false;
				lock.notify();
	...
	public void register(MediaKey mediaKey, HotKeyListener listener) {
		Logger.get().log(Level.WARNING, "Media keys are not supported on this platform");
	}
	...
	private class EventHandler implements Carbon.EventHandlerProcPtr {
		public int callback(Pointer inHandlerCallRef, Pointer inEvent, Pointer inUserData) {
			...
			if (ret != 0) {
				Logger.get().log(Level.WARNING, "Could not get event parameters. Error code: " + ret);
			} else {
				int eventId = eventHotKeyID.id;
				Logger.get().log(Level.INFO, "Received event id: " + eventId);
