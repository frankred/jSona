...
public class WindowsProvider extends Provider {
	...
	public void init() {
		Runnable runnable = new Runnable() {
			public void run() {
				Logger.get().log(Level.INFO, "Starting Windows global hotkey provider");
				MSG msg = new MSG();
				listen = true;
				while (listen) {
				...
				Logger.get().log(Level.INFO, "Exit hotkey thread.");
			}
		};
	...
	private void register(HotKey hotKey) {
		...
		if (RegisterHotKey(null, id, KeyMap.getModifiers(hotKey.keyStroke), code)) {
			...
			Logger.get().log(Level.WARNING, "Could not register hotkey: " + hotKey);
		}
	}
